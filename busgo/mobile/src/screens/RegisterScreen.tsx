...
export default function RegisterScreen({ navigation }: ScreenProps<'Register'>) {
  ...
  const register = async () => { setBusy(true); try { await signInWithGoogle(); } catch (error: any) { if (error?.message !== 'Google login was cancelled.') Alert.alert('Registration failed', error?.message || 'Please try again.'); } finally { setBusy(false); } };
