...
export default function LoginScreen({ navigation }: ScreenProps<'Login'>) {
  ...
  const signIn = async () => { setBusy(true); try { await signInWithGoogle(); } catch (error: any) { if (error?.message !== 'Google login was cancelled.') Alert.alert('Google login failed', error?.message || 'Please try again.'); } finally { setBusy(false); } };
  return <ScrollView style={{ flex: 1, backgroundColor: colors.dark }} contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 24 }}>
    ...
    <Card><Text style={{ fontSize: 21, fontWeight: '900', color: colors.text }}>Welcome back</Text><Text style={{ color: colors.subtext, lineHeight: 20, marginTop: 6, marginBottom: 20 }}>Sign in with the Google account connected to BusGo. Your bookings and tickets are restored automatically.</Text><Button title="Continue with Google" icon="logo-google" onPress={signIn} loading={busy} /><Button title="Create a passenger account" variant="ghost" onPress={() => navigation.navigate('Register')} style={{ marginTop: 6 }} /></Card>
