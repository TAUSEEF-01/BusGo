...
export default function LoginScreen({ navigation, route }: ScreenProps<'Login'>) {
  ...
  return <ScrollView style={{ flex: 1, backgroundColor: colors.dark }} contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 24 }}><StatusBar style="light" />
    ...
    <Card><Text style={{ fontSize: 21, fontWeight: '900', color: colors.text }}>Welcome back</Text><Text style={{ color: colors.subtext, lineHeight: 20, marginTop: 6, marginBottom: 20 }}>{resumeCheckout ? 'Sign in to keep your selected seats and continue checkout.' : 'Sign in with Google — your BusGo account is created automatically the first time, and your bookings and tickets are restored.'}</Text><Button title="Continue with Google" icon="logo-google" onPress={signIn} loading={busy} /><Button title="Continue browsing" variant="ghost" onPress={() => navigation.popToTop()} /></Card>
