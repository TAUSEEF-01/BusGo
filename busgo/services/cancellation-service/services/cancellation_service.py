...
def process_cancellation(db: Session, request: CancellationCreate, user_id):
    ...
    if 23 <= hour or hour < 7:
        raise HTTPException(status_code=400, detail="Cancellations are not allowed between 11 PM and 7 AM")
        
    departure = booking["departure_time"]
    time_diff = (departure - now).total_seconds() / 3600
    
    if time_diff < 12:
        raise HTTPException(status_code=400, detail="Cannot cancel less than 12 hours before departure")
    ...
    # Publish to Kafka
    publish_message("booking.cancelled", {
...
def process_operator_cancellation(db: Session, request: OperatorCancellationCreate):
    ...
    bookings = [{"id": "11111111-1111-1111-1111-111111111111", "user_id": "00000000-0000-0000-0000-000000000000", "total_amount": 100.0}]
...
def get_cancellation_by_booking(db: Session, booking_id):
    return db.query(CancellationRequest).filter(CancellationRequest.booking_id == booking_id).first()
